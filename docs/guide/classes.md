...
The decorator form `@deprecated_class(target=BrandColor, ...)` behaves identically at runtime but stays `Deprecated[Any]` for the type checker — mypy does not rebind a `class` statement to an instance return type. Reach for the functional form when you want the target type to flow into call sites; otherwise the decorator form is the more readable default.
