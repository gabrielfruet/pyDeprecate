...
The decorator form `@deprecated_class(target=RetryPolicy, ...)` behaves identically at runtime but stays `Deprecated[Any]` — mypy does not rebind a `class` statement to an instance return type, so there is nothing for it to infer `T` from.
